...
Room invites: tapping either room code copies the invite URL. The header QR button opens an accessible dialog with a locally generated QR, room code, and copyable URL. No seat token is included. `public/qrcode.js` is vendored from `qrcode-generator` 2.0.4; preserve the notices when updating it. `node tests/invite.mjs` checks the dialog in Chromium/WebKit, independently decodes the generated QR with jsQR, follows that link to join, and tests clipboard success/failure via a browser stub.
