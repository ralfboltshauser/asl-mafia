...
Room invites: tapping either room code copies the invite URL. The QR button beside the code in the invite card opens an accessible dialog with a locally generated QR, room code, and copyable URL. No seat token is included. `public/qrcode.js` is vendored from `qrcode-generator` 2.0.4; preserve the notices when updating it. `node tests/invite.mjs` checks the dialog in Chromium/WebKit, independently decodes the generated QR with jsQR, follows that link to join, and tests clipboard success/failure via a browser stub.

Angel self-protection: choose “After protecting yourself” under “The limit counts…”. With alternate nights, only self-protection rests the following night; other targets remain available. With once per game, only self-protection is consumed. Choosing self counts even if the mafia attacked someone else. Skipping or protecting others does not consume the limit. Settings remain lobby-only. `node tests/angel-self.mjs` exercises three real nights through the UI/API.
