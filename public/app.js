...
function progress(){
 if(s.phase==='night')return `<div class="progress"><span>Night ${s.round}</span><span>${s.nightCue?roleNames[s.nightCue]+' turn':'Waiting for night actions'}</span></div>`;
...
function secret(){return revealed?`<div class="secret-card"><span class="secret-label">PRIVATE</span>${s.me.investigation?`<p class="investigation-result"><strong>${esc(s.me.investigation.name)} is ${s.me.investigation.mafia?'mafia':'not mafia'}.</strong></p>`:''}<h2>${roleNames[s.me.role]}</h2><p>${s.me.role==='angel'?angelDescription():roleCopy[s.me.role]}</p>${s.me.role==='mafia'?`<p><strong>${s.team.length?`Your team: ${s.team.map(p=>esc(p.name)).join(', ')}`:'You are the only mafia player.'}</strong></p>`:''}</div>${button('Hide private information','hide','private-toggle')}`:button('View private information','reveal','private-toggle');}
...
function render(){
 ...
 html+=`<p class="muted">${s.me.role==='mafia'?'Your team’s most-picked target is eliminated.':s.me.role==='sheriff'?'You’ll get a private result at dawn.':'You may protect yourself or skip this night.'}</p>${s.me.angelTarget?`<p class="target-notice">Mafia’s target: <strong>${esc(s.me.angelTarget)}</strong></p>`:''}`;
 const opts=s.players.filter(p=>p.alive&&(p.id!==s.me.id||s.me.role==='angel')&&(s.me.role!=='mafia'||!s.team.some(t=>t.id===p.id)));
 ...
 html+=`<h1>${!s.me.alive?'You’re out':s.me.submitted?'Vote submitted':'Who should be voted out?'}</h1><p class="muted">${!s.me.alive?'Waiting for the living players to vote.':s.me.submitted?'Your vote is locked. Waiting for the remaining players.':s.config.dayVoteVisibility==='public'?'Everyone will see your vote after voting ends.':'Your vote is secret.'}</p>${s.me.alive&&!s.me.submitted?choices(s.players.filter(p=>p.alive),true)+button('Lock my vote','choose','primary',!choice)+'<p class="caption">You may vote for yourself. Votes are final; a tie eliminates nobody.</p>':''}${progress()}`;
 }
 if(s.phase==='result'){
 html+=`<h1>${s.report.name?`${esc(s.report.name)} was voted out.`:'Nobody was voted out.'}</h1><p class="muted">${s.report.name?'Their role stays secret until the game ends.':s.report.method==='in-person'?'The group chose not to eliminate anyone.':'The vote was tied or abstain received the most votes.'}</p>${s.report.method==='in-person'?'<p class="caption">Confirmed by the room creator.</p>':tally()}${host()?button('Begin the next night','night'):'<p class="caption">Waiting for the creator to begin the next night.</p>'}`;
 }
 if(s.phase==='over'){
...
app.addEventListener('click',async e=>{const b=e.target.closest('button');if(!b||busy)return;if(b.dataset.mode){const name=document.querySelector('#name')?.value;mode=b.dataset.mode;render();document.querySelector('#name').value=name;return;}if(b.dataset.choice){choice=b.dataset.choice;render();return;}if(b.dataset.remove){if(confirm('Remove this player from the lobby?'))send('remove',{target:b.dataset.remove});return;}const a=b.dataset.action;if(!a)return;if(a==='resume'){paused=false;poll();return;}if(a==='reveal'||a==='hide'){revealed=a==='reveal';render();return;}if(a==='copy-link'){await copyInvite();return;}if(a==='copy'){const link=`${location.origin}/?room=${s.code}`;try{if(navigator.share){try{await navigator.share({title:'Join my Mafia room',text:`Room ${s.code} — ASL Mafia`,url:link});return;}catch(e){if(e.name==='AbortError')return;}}await navigator.clipboard.writeText(link);b.textContent='Invite link copied ✓';}catch{error.textContent=`Share this link: ${link}`;}return;}if(a==='resolve-vote'){const target=choice;if(!target)return;const name=s.players.find(p=>p.id===target)?.name;const text=target==='skip'?'Confirm the group’s result: nobody is eliminated?':`Confirm the group’s result: eliminate ${name}?`;if(confirm(`${text}\n\nThis is final and may end the game.`))send('resolve-vote',{target,confirmed:true});return;}if(a==='sleep'){send('choose',{target:'sleep'});return;}if(a==='choose'){send('choose',{target:choice});return;}send(a);});
...
document.querySelector('#invite-qr').onclick=showInvite;
