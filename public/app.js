...
function accept(data){if(s&&s.code===data.state.code&&s.revision>data.state.revision)return;const oldStage=s?.stage,oldCue=s?.nightCue;restoring=false;restoreFailed=false;s=data.state;paused=false;if(data.token){seat={token:data.token,code:s.code};try{localStorage.setItem(storageKey,JSON.stringify(seat));}catch{status('Your browser cannot save your seat. Keep this tab open.');}}
...
document.querySelector('#exit').onclick=()=>{if(confirm('Leave this screen? Your seat stays in this browser; use Resume to return.')){paused=true;restoring=false;restoreFailed=false;nightAudio.stop();s=null;lastJson='';render();}};
...
restoring=!!seat;render();if(seat)poll();setInterval(poll,2500);
