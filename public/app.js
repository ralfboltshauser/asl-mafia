...
let paused=false,seat=null,s=null,mode=new URLSearchParams(location.search).has('room')?'join':'create',revealed=false,choice=null,busy=false,lastJson='',lastAnnounced=null,paintedStage=null,settingUndo=null;
...
async function send(action,extra={}){
 ...
 busy=true;error.textContent='';app.querySelectorAll('button,input,select').forEach(b=>b.disabled=true);
 try{accept(await request(action,extra));if(action==='ready')revealed=false;
  ...
  status('');return true;
 }catch(e){error.textContent=e.message;return false;}finally{busy=false;render();}
}
...
app.addEventListener('submit',e=>{if(e.target.id!=='entry')return;e.preventDefault();if(seat&&!confirm('Replace your saved seat with a different room? You will no longer be able to resume the old seat from this browser.'))return;send(mode,{name:document.querySelector('#name').value,...(mode==='join'?{code:document.querySelector('#code').value.trim().toUpperCase()}: {})});});
...
app.addEventListener('click',async e=>{const b=e.target.closest('button');if(!b||busy)return;if(b.dataset.mode){const name=document.querySelector('#name')?.value;mode=b.dataset.mode;render();document.querySelector('#name').value=name;return;}if(b.dataset.choice){choice=choice===b.dataset.choice?null:b.dataset.choice;render();return;}if(b.dataset.remove){if(confirm(`Remove ${s.players.find(p=>p.id===b.dataset.remove)?.name} from this room? They will need to join again. This cannot restore their old seat.`))send('remove',{target:b.dataset.remove});return;}const a=b.dataset.action;if(!a)return;if(a==='clear-choice'){choice=null;render();return;}if(a==='undo-settings'){if(canUndoSettings())send('configure',{config:settingUndo.before,expectedConfig:settingUndo.after});return;}if(a==='resume'){paused=false;poll();return;}if(a==='reveal'||a==='hide'){revealed=a==='reveal';render();return;}if(a==='qr'){showInvite();return;}if(a==='copy-link'){await copyInvite();return;}if(a==='copy'){const link=`${location.origin}/?room=${s.code}`;try{if(navigator.share){try{await navigator.share({title:'Join my Mafia room',text:`Room ${s.code} — ASL Mafia`,url:link});return;}catch(e){if(e.name==='AbortError')return;}}await navigator.clipboard.writeText(link);b.textContent='Invite link copied ✓';}catch{error.textContent=`Share this link: ${link}`;}return;}if(a==='resolve-vote'){const target=choice;if(!target)return;const name=s.players.find(p=>p.id===target)?.name;const text=target==='skip'?'Confirm the group’s result: nobody is eliminated?':`Confirm the group’s result: eliminate ${name}?`;if(confirm(`${text}\n\nThis is final and may end the game.`))send('resolve-vote',{target,confirmed:true});return;}if(a==='sleep'){if(confirm('Skip protection this night? Nobody will receive your protection. This cannot be changed after you submit.'))send('choose',{target:'sleep'});return;}if(a==='choose'){if(!choice)return;const name=s.players.find(p=>p.id===choice)?.name;const verb=s.phase==='vote'?'Vote for':s.me.role==='mafia'?'Choose as the mafia target:':s.me.role==='sheriff'?'Investigate':'Protect';const message=choice==='skip'?'Abstain from this vote?':`${verb} ${name}${choice===s.me.id?' (yourself)':''}?`;if(confirm(`${message}\n\nYour choice is final. The game may advance immediately.`))send('choose',{target:choice});return;}const warning={start:'Start the game and deal secret roles? Players and settings will be locked until the game ends.',ready:'Ready to begin? If you are the last player, the night starts immediately. You can undo readiness only while others are still getting ready.',vote:'End the discussion and start voting? You cannot return to the discussion phase.',night:'Start the next night? The previous result stays final.',restart:'Return everyone to the lobby? The finished game and roles will be cleared. You can adjust settings before dealing again.'}[a];if(warning&&!confirm(warning))return;send(a);});
...
document.querySelector('#audio-mode').onchange=async e=>{const mode=e.target.value;if(mode!=='off'&&!s.guidedNext&&!confirm('Enable guided turns for future nights? Roles will act one at a time. Turning audio off later only mutes sound; guided turns stay enabled.')){e.target.value=nightAudio.mode;return;}await nightAudio.enable(mode);e.target.value=nightAudio.mode;if(nightAudio.mode!=='off'){if(!s.guidedNext)await send('guide');nightAudio.event=null;nightAudio.update(s);if(!s.nightCue&&s.report?.kind!=='night')nightAudio.play('ready');}};
