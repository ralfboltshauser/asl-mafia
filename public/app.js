...
let inviteNameFocusPending=mode==='join';
...
function accept(data){if(s&&s.code===data.state.code&&s.revision>data.state.revision)return;const oldStage=s?.stage,oldCue=s?.nightCue;restoring=false;restoreFailed=false;s=data.state;inviteNameFocusPending=false;if(confirmation&&confirmation.context!==confirmationContext())confirmDialog.close('cancel');paused=false;if(data.token){seat={token:data.token,code:s.code};try{localStorage.setItem(storageKey,JSON.stringify(seat));}catch{status('Your browser cannot save your seat. Keep this tab open.');}}
...
function secret(){return revealed?`<div class="secret-card">${s.me.investigation?`<p class="investigation-result"><strong>${esc(s.me.investigation.name)} is ${s.me.investigation.mafia?'mafia':'not mafia'}.</strong></p>`:''}${s.phase==='roles'?'':`<h2>${roleNames[s.me.role]}</h2>`}<p>${s.me.role==='angel'?angelDescription():roleCopy[s.me.role]}</p>${s.me.role==='mafia'?`<p><strong>${s.team.length?`Your team: ${s.team.map(p=>esc(p.name)).join(', ')}`:'You are the only mafia player.'}</strong></p>`:''}</div>${s.phase==='roles'?'':button('Hide private information','hide','private-toggle')}`:button('View private information','reveal',s.phase==='roles'&&!s.me.submitted?'primary':'private-toggle');}
...
function paint(html){
 ...
 if(inviteNameFocusPending&&!s&&mode==='join'&&app.querySelector('#name')){inviteNameFocusPending=false;const name=app.querySelector('#name');name.focus({preventScroll:true});name.removeAttribute('autofocus');}
 paintedStage=stage;
}
function render(){
 ...
 if(s.phase==='roles'){
 html+=`<h1>${revealed?`You’re ${s.me.role==='town'?'a townsperson':s.me.role==='angel'?'the angel':s.me.role==='sheriff'?'the sheriff':'mafia'}.`:s.me.submitted?'Waiting for players':'Check your role'}</h1>${!s.me.submitted&&!revealed?'<p class="muted">Make sure nobody can see your screen.</p>':''}${secret()}${revealed?button('I know my role','ready'):s.me.submitted?button('Undo ready','unready','secondary'):''}${progress()}`;
 }
 if(s.phase==='night'){
...
const helpText={
 ...
 ready:'Hides your role and marks you ready. You can undo while others are still getting ready. Once everyone is ready, the night starts automatically and readiness cannot be undone.',
 reveal:'Shows only your own role or night action. Keep your screen private. During role onboarding, I know my role hides it and marks you ready. During play, you can hide private information at any time.',
