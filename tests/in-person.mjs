...
try{
 ...
 await page.getByRole('button',{name:'Deal secret roles'}).click();let state=(await api(0,'state')).state;
 ...
 await page.locator('[data-choice="skip"]').click();page.once('dialog',async dialog=>{assert.match(dialog.message(),/nobody is eliminated/);await dialog.accept();});await page.getByRole('button',{name:'Lock group result'}).click();await page.getByRole('button',{name:'Begin the next night'}).waitFor();state=(await api(0,'state')).state;assert.equal(state.report.name,null);assert.equal(state.report.method,'in-person');assert.equal(state.report.tally,undefined);
 ...
 await page.getByRole('button',{name:'Begin the next night'}).click();await night();await page.getByRole('button',{name:'Begin in-person vote'}).click();await page.locator(`[data-choice="${states[mafia].me.id}"]`).click();page.once('dialog',dialog=>dialog.accept());await page.getByRole('button',{name:'Lock group result'}).click();await page.getByRole('button',{name:'Play again'}).waitFor();state=(await api(0,'state')).state;assert.equal(state.winner,'town');assert.equal(state.report.name,states[mafia].me.name);assert.equal(state.report.ballots,undefined);assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth>innerWidth),false);assert.deepEqual(errors,[]);
