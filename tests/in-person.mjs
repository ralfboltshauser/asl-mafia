...
try{
 const context=await browser.newContext({viewport:{width:390,height:844},isMobile:true,hasTouch:true});const page=await context.newPage();await autoConfirm(page,{exceptGroup:true});const errors=[];page.on('pageerror',e=>errors.push(e.message));
 ...
 await page.getByRole('button',{name:'Start game'}).click();await page.locator('#app[data-phase=roles]').waitFor();let state=(await api(0,'state')).state;
 ...
 await page.locator(`[data-choice="${state.me.id}"]`).click();await page.getByRole('button',{name:'Lock group result'}).click();await page.locator('#confirm-dialog[open]').waitFor();assert.match(await page.locator('#confirm-title').textContent(),/eliminate Manual host/);await page.locator('#confirm-cancel').click();assert.equal((await api(0,'state')).state.phase,'vote');
 await page.locator('[data-choice="skip"]').click();await page.getByRole('button',{name:'Lock group result'}).click();await page.locator('#confirm-dialog[open]').waitFor();assert.match(await page.locator('#confirm-title').textContent(),/nobody is eliminated/);await page.locator('#confirm-accept').click();await page.getByRole('button',{name:'Start next night'}).waitFor();state=(await api(0,'state')).state;assert.equal(state.report.name,null);assert.equal(state.report.method,'in-person');assert.equal(state.report.tally,undefined);
 ...
 await page.getByRole('button',{name:'Start next night'}).click();await page.locator('#app[data-phase=night]').waitFor();await night();await page.getByRole('button',{name:'Begin in-person vote'}).click();await page.locator(`[data-choice="${states[mafia].me.id}"]`).click();await page.getByRole('button',{name:'Lock group result'}).click();await page.locator('#confirm-accept').click();await page.getByRole('button',{name:'Play again'}).waitFor();state=(await api(0,'state')).state;assert.equal(state.winner,'town');assert.equal(state.report.name,states[mafia].me.name);assert.equal(state.report.ballots,undefined);assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth>innerWidth),false);assert.deepEqual(errors,[]);
