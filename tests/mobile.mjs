...
try{
 for(const [width,height] of [[320,568],[375,667],[390,844],[430,932],[844,390]].filter(([w])=>!process.env.WIDTH||w===Number(process.env.WIDTH))){
  ...
  async function update(phase){state.phase=phase;state.stage++;state.revision++;await page.evaluate(()=>document.dispatchEvent(new Event('visibilitychange')));await page.waitForFunction(phase=>document.querySelector('#app')?.dataset.phase===phase,phase);}
